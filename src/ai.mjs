...
import { capabilitiesLine, featureState, stylePref, toneBlock, questionHabit, stripTrailingQuestion, voiceStateLine } from './capabilities.mjs';
...
export function friendSystem(user) {
  ...
  if (user?.isGroup) {
    return (
      ...
      'НО если просят СОЗДАТЬ контент - план тренировки, список, рецепт, текст, идеи, код - это ты умеешь и выдаёшь ПРЯМО в ответе, сразу и целиком. НИКОГДА не пиши «сейчас отправлю», «уже пишу», «держи, сейчас скину» - выдавай результат немедленно в этом же сообщении, не обещай прислать потом. Не откладывай на «завтра» и не отговаривайся временем суток или усталостью - выдавай сейчас; не хватает деталей - прими разумные умолчания и всё равно дай полный результат. ' +
