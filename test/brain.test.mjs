...
test('«очисти память» - это команда очистки, а не заметка', async () => {
  ...
  assert.equal(parseMessage('очисти память').kind, 'clearchat');
  assert.equal(parseMessage('очистить память').kind, 'clearchat');
  assert.equal(parseMessage('сотри переписку').kind, 'clearchat');
