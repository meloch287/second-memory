...
export function startTelegramBot(store, token, log = console) {
  ...
  function helpText(user) {
    ...
    return [
      ...
      '',
      `<blockquote>🗣 Как я говорю: по умолчанию - как свой пацан, со сленгом и матом. Не заходит - скажи «${esc(botNameOf(user))}, не общайся матом», и мат уберу (живость останется). Вернуть - «можешь материться». Нужен деловой тон - «общайся официально». Ещё понимаю «называй меня братан», «не называй меня по имени» и «не задавай встречных вопросов».</blockquote>`,
      '',
