...
// Как бот РАЗГОВАРИВАЕТ. По умолчанию - живой пацан в чате, мат в порядке
// вещей. Отключается только явной просьбой («Толя, не общайся матом») - тогда
// talkStyle='clean'; 'official' - деловой тон на «вы».
//
...
export function toneBlock(user) {
  ...
  if (user?.talkStyle === 'clean') {
    return (
      TEEN_MECHANICS +
      'МАТ ВЫКЛЮЧЕН: человек попросил не материться - не используй мат и грубые слова совсем, даже мягкие. ' +
      'Живость сохраняй: сленг, подколы и юмор остаются, просто без мата. ' +
      'Если тебя дразнят или просят дурашливое - подыграй и отшутись, не читай нотаций. '
    );
  }
  return (
    TEEN_MECHANICS +
    'МАТ - ЧАСТЬ ТВОЕЙ ОБЫЧНОЙ РЕЧИ. Материшься свободно и естественно, как свой человек: бля, блин, нахуй, пиздец, ахуеть, ебать, хуйня, заебись, охуенно. ' +
    'Но: мат для экспрессии, а НЕ в адрес собеседника - его не оскорбляй и не унижай. Не пихай мат в каждое слово, звучи естественно. ' +
    'Если тебя дразнят, подкалывают или просят дурашливое - подыграй, отшутись, можешь подколоть в ответ. Нотаций не читай. ' +
    'Не превращайся в канцелярит: не пересказывай просьбу словами «я понял, что нужно...» и не изображай, что физически идёшь её выполнять. '
  );
}
...
export function stylePref(user) {
  ...
  if (user?.talkStyle === 'clean') {
    parts.push('НАПОМИНАНИЕ: человек просил не материться. Ни одного матерного или грубого слова, даже мягкого.');
  }
  if (user?.addressAs) {
...
// Мат ВКЛЮЧЁН по умолчанию, поэтому ловим прежде всего просьбу его убрать.
// «Толя, не общайся матом» - главная формулировка из /help.
const RE_NO_MAT =
  /(?:не\s+(?:матерись|ругайся|матерись)|без\s+мата|хватит\s+материться|перестань\s+материться|не\s+обща(?:йся|ться)[а-яё\s]{0,10}мат|не\s+ругайся\s+матом|убери\s+мат|выключи\s+мат)/i;
// Возврат к дефолту: «можешь материться», «включи мат», «общайся как раньше».
const RE_MAT =
  /(?:можешь|можно|разрешаю|включи|верни)[а-яё\s]{0,15}(?:материться|мат[ое]м?|ругаться|мат)|обща(?:йся|ться)[а-яё\s]{0,10}мат[ое]м|мат(?:ерись|юкайся)/i;
...
export function parseStylePref(text) {
  ...
  // Порядок важен: «не общайся матом» содержит и «общайся мат...», поэтому
  // запрет проверяем ПЕРВЫМ, иначе он распознается как разрешение.
  if (RE_NO_MAT.test(t)) patch.talkStyle = 'clean';
  else if (RE_OFFICIAL.test(t)) patch.talkStyle = 'official';
  else if (RE_MAT.test(t) || RE_CASUAL.test(t)) patch.talkStyle = null; // назад к дефолту (мат включён)
...
export function captureStylePref(store, chatId, text) {
  ...
  if (patch.talkStyle === 'clean') return 'лан, без мата';
