...
export function parseMessage(text, now = new Date()) {
  ...
  // «посмотри курс доллара» раньше не ловилось: требовалось начало строки,
  // и вопрос уходил в ИИ, который выдумывал число.
  // «курс» в середине фразы считается валютным только рядом с самой валютой -
  // иначе «я на курсе английского» уезжало в котировки.
  const moneyWord = /доллар|евро|юан|тенге|фунт|франк|лир|бакс|валют|usd|eur|cny|\$|€/i.test(t);
  if (
    /^курс[ыае]?(?![а-яё])/.test(t) ||
    (/(?:^|[\s,])курс[ыае]?(?![а-яё])/.test(t) && moneyWord) ||
    /(?:\d|\$|€)\s*(?:\$|€|доллар|евро|юан|тенге|фунт|франк|лир)[а-я]*\s+в\s+руб/.test(t)
  ) {
...
// Глагол-напоминание побеждает любую другую классификацию (см. parseEntry).
const REMINDER_VERB = /(?:^|[^а-яё])(?:напомни(?:ть)?|напоминани[ея]|переспрос[а-я]*|спроси\s+меня|не\s+забудь\s+спросить)(?![а-яё])/;
// «должно/должны + прийти/поступить/зачислить» - ожидание дохода, не долг.
const INCOME_NOT_DEBT = /должн[а-я]*\s+(?:прийти|прийдет|прийдёт|придет|придёт|поступить|поступит|зачисли[а-я]*)/;
...
function parseEntry(raw, t, now) {
  ...
  // «напомни / напомнить / переспроси / спроси меня / не забудь спросить» -
  // это ВСЕГДА задача-напоминание, даже если во фразе есть «долг/должен/
  // должно» (баг: «напомни мне 29 июля, что должно прийти 95к по зп»
  // классифицировалось как долг и теряло указанную дату). Проверяем ДО
  // debt-ветки, чтобы глагол-напоминание перебивал долговые слова.
  if (REMINDER_VERB.test(t)) {
  ...
  // «должно прийти / поступить / зачислят» - ожидание ДОХОДА, а не долг
  // пользователя (баг: «мне должно прийти 95к по зп» записывалось как долг).
  if (/долж|долг|задолж|одолжил|занял/.test(t) && !INCOME_NOT_DEBT.test(t)) {
...
// Слова, которые не могут быть именем контрагента после «одолжил» / после «у».
const DEBT_STOP = new Set([
  'у', 'мне', 'нам', 'тебе', 'вам', 'ему', 'ей', 'им', 'себе',
  'меня', 'нас', 'тебя', 'вас', 'него', 'нее', 'неё', 'них',
  'немного', 'чуть', 'ещё', 'еще', 'свои', 'свою', 'деньги', 'денег',
]);
// Слова перед «занял/одолжил», которые не могут быть именем (подлежащее-местоимение и пр.).
const DEBT_SUBJ_STOP = new Set([
  'я', 'мы', 'он', 'она', 'они', 'ты', 'вы', 'кто',
  'вчера', 'сегодня', 'завтра', 'тоже', 'также', 'уже', 'снова', 'опять',
]);
...
function parseDebt(raw, t, dt, due) {
  ...
  // «занял/одолжил» — направление зависит от порядка слов; раньше direction
  // смотрел только на корень «долж» и терял/инвертировал эти фразы.
  const borrowVerb =
    /занял|заняла|одолжил|одолжила/.test(t) ||
    /(^|\s)взял[аи]?\s+(?:в\s+долг|денег|деньги)/.test(t);
  // Имя ПЕРЕД глаголом: «Дима одолжил…», «Петя занял…» (не местоимение).
  const before =
    (m = t.match(/(?:^|\s)([а-яёa-z-]{3,})\s+(?:занял|заняла|одолжил|одолжила)/)) &&
    !DEBT_SUBJ_STOP.has(m[1]) && !DEBT_STOP.has(m[1])
      ? cap(m[1])
      : null;

  if (borrowVerb && /(?:^|\s)у\s+(?:меня|нас)(?![а-яё])/.test(t)) {
    // «Петя занял у меня» — взял у меня → должен мне (in), контрагент — кто взял.
    direction = 'in';
    counterparty = before || findCounterparty(rest);
  } else if (before && /(?:одолжил|одолжила)\s+(?:мне|нам)(?![а-яё])/.test(t)) {
    // «Дима одолжил мне» — дал мне в долг → я должен Диме (out).
    direction = 'out';
    counterparty = before;
  } else if (
    borrowVerb &&
    (m = t.match(/(?:^|\s)у\s+([а-яёa-z][а-яёa-z-]+)/)) &&
    !DEBT_STOP.has(m[1])
  ) {
    // «я занял у Пети», «одолжил у Игоря» — взял у X → я должен (out).
    direction = 'out';
    ...
    direction = 'in';
    counterparty = cap(m[1]);
  } else if (/(?:^|\s)(?:я|мы)\s+занял[аи]?(?![а-яё])/.test(t) || /^занял[аи]?(?![а-яё])/.test(t)) {
    // «я занял 5000» без имени — всё равно взял в долг → out.
    direction = 'out';
