...
test('шифрование: файл нечитаем без ключа, читается с ключом, миграция', () => {
  ...
  try {
    ...
    s.flush(); // запись дебаунснута — дописываем перед чтением файла
    const raw = readFileSync(file);
    assert.ok(raw.subarray(0, 6).toString() === 'SMENC2', 'файл в формате SMENC2 (scrypt+соль)');
    ...
    // два сохранения дают разные шифртексты (случайная соль+IV)
    s2.save(); s2.flush();
    assert.ok(!readFileSync(file).equals(raw), 'соль/IV случайные - файл меняется');
  } finally {
    delete process.env.SM_ENCRYPTION_KEY;
  }
});
...
test('шифрование: ключ кэшируется (соль стабильна), но IV уникален на каждую запись', () => {
  const file = tmpFile();
  process.env.SM_ENCRYPTION_KEY = 'test-secret-key';
  try {
    const s = new Store(file);
    s.add({ type: 'note', title: 'a', chatId: '1' });
    s.flush();
    const f1 = readFileSync(file);
    s.add({ type: 'note', title: 'b', chatId: '1' });
    s.flush();
    const f2 = readFileSync(file);
    const salt = (b) => b.subarray(7, 23).toString('hex'); // 16 байт соли после magic
    const iv = (b) => b.subarray(23, 35).toString('hex'); // 12 байт IV
    assert.equal(salt(f1), salt(f2), 'соль кэширована в рамках процесса');
    assert.notEqual(iv(f1), iv(f2), 'IV случаен на каждую запись (GCM-nonce уникален)');
    assert.ok(!f1.equals(f2), 'файл всё равно меняется между записями');
    // и файл по-прежнему читается тем же ключом
    const s2 = new Store(file);
    assert.equal(s2.list({ chatId: '1' }).length, 2);
  } finally {
    delete process.env.SM_ENCRYPTION_KEY;
  }
});
