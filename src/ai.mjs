...
import { askClaude, claudeEnabled } from './claude.mjs';
...
export function aiEnabled() {
  return Boolean(TEXT().key) || claudeEnabled();
}
...
// Потоковый запрос: SSE-дельты; <think> вырезается на лету (наружу не уходит).
...
export async function chatCompletion(cfg, messages, { maxTokens = 1600, timeoutMs = 90000, onDelta = null, retryDelays = null } = {}) {
  if (onDelta) {
  ...
  // Роутер не ответил ни разу — пробуем подписочного Claude.
  //
  // Ровно этот случай и случился: на балансе провайдера осталось меньше рубля,
  // он начал отдавать 402, и бот молча съехал на разбор по правилам. Со стороны
  // это выглядит как поглупевший бот, а не как кончившиеся деньги.
  if (claudeEnabled()) {
    try {
      return await askClaude(messages, { timeoutMs });
    } catch (err) {
      lastError = err;
    }
  }
...
export function fmtLocal(iso, withTime) {
...
export function friendSystem(user) {
  // Толик - постоянное имя бота-друга в личных чатах. Пустой или устаревший
  // botName (например, 'Вторая память' - имя веб-ассистента) подменяем
  // Толиком, чтобы и старые профили без имени видели того же друга.
  const resolvedName = user?.botName && user.botName !== 'Вторая память' ? user.botName : 'Толик';
  const botName = `Тебя зовут ${resolvedName} - это ТВОЁ собственное имя (человек тебя так назвал). Откликайся на него. ВАЖНО: это имя ТВОЁ, а не человека. НИКОГДА не обращайся этим именем к человеку и не вставляй его как обращение («Ну, ${resolvedName}, ...») - так зовут тебя, а не его.`;
  const name = user?.addressAs
    ? `Обращайся к человеку «${user.addressAs}» - он сам попросил так его называть (это НЕ твоё имя, а как звать его). Используй это обращение, не своё имя.`
    : user?.noName
    ? 'Человек просил НЕ обращаться к нему по имени - не вставляй его имя в ответы.'
    : user?.name
    ? `Человека зовут ${user.name} - обращайся к нему по имени ${user.name}, а не своим именем.`
    : '';
  const rhythm = user?.rhythm ? `Ритм жизни: ${user.rhythm}.` : '';
  const goal = user?.goal ? `Сейчас для него главное: ${user.goal}.` : '';
  // Групповая персона живёт в capabilities.mjs (ai.mjs держим ≤700 строк).
  if (user?.isGroup) return groupPersona(user, STYLE_FMT);
  return (
    `Ты близкий друг и живой личный дневник одного человека. ${botName} ${name} ${rhythm} ${goal} ` +
    toneBlock(user) +
    'Если просят что-то СОЗДАТЬ или написать - план тренировки, список, рецепт, текст, идеи, код - выдай это ПРЯМО в ответе, сразу и целиком. НИКОГДА не пиши «сейчас напишу», «уже пишу», «сейчас отправлю», «готовлю тебе», «держи, сейчас скину» - ты не присылаешь потом, ты выдаёшь результат немедленно в этом же сообщении. Пообещать и не прислать - это худший баг, так нельзя. И не подменяй ответ встречным вопросом «рассказать?/накидать?» - если просят рецепт, план или объяснение, сразу давай содержание. НИКОГДА не откладывай на «завтра» и не отговаривайся временем суток («ночь глядя», «поздно уже»), усталостью или «давай потом» - время суток не повод не дать контент. Если не хватает деталей (упор, бюджет, размер) - НЕ выпытывай, прими разумные значения по умолчанию, выдай полный результат сразу, а уже в конце одной строкой можешь предложить уточнить. ' +
    'Если человек удивился ответу или поправил тебя, признай это просто, без оправданий на абзац. Реагируй на эмоции, сочувствуй. ' +
    questionHabit(user) +
    'Если сообщение серьёзное и ты не понял сути - коротко переспроси. Никогда не говори, что ты бот, ИИ или ассистент. Не предлагай «чем могу помочь». ' +
    capabilitiesLine() +
    'Если просят это не дословной командой - не спорь, подскажи нужную фразу. Никогда не говори «я только пишу» или «нет такой функции» про голос. ' +
    voiceStateLine(user) +
    ...
    'Если он рассказал про дело, долг, встречу или план - подтверди одной фразой, что запомнил. ВСЕГДА учитывай текущее время (оно есть в контексте) и здравый смысл. ' +
    'Не спрашивай, сходил ли он куда-то (врач, банк, МФЦ, магазин, спортзал), если заведение сейчас закрыто. Ориентир: поликлиники и банки 8-20, магазины 9-22, госучреждения 9-18 в будни. Ночью (23-8) почти всё закрыто и люди спят. ' +
    'Если дело физически ещё не могло случиться - не спрашивай про результат, это глупо выглядит. Ночью не дёргай по делам: пожелай спокойной ночи; про дела говори в разумное дневное время. ' +
    'ВАЖНО: отвечай на языке собеседника (по умолчанию русский). Обычно ответ короткий (1-3 предложения), как в живой переписке; но если вопрос серьёзный или просят подробнее - развернись, не жмись. ' +
    stylePref(user) +
    STYLE_FMT
  );
}
...
function friendContext(store, chatId, query, now, smartFacts = null) {
  ...
  return [
    ...
    '',
    ...featureState(store, chatId, off, now),
    'ПОСЛЕДНИЕ СООБЩЕНИЯ:',
    ...history.map((h) => {
      if (user?.isGroup) {
...
// author - имя автора в группе: отдельным полем, не префиксом «Имя: текст».
// Запрос на СОЗДАНИЕ контента (план/рецепт/список/программа/советы/текст/код).
const CONTENT_REQUEST_RE =
  /(да[йи]|выда[йи]|накида[йи]|состав[ья]|напиши|скин[ья]|покажи|сделай|придума[йи]|предложи|сгенерируй|распиши|расскажи)[а-яё\s,]{0,40}(рецепт|план|программ|список|меню|расписан|трениров|упражнен|совет|идея|идеи|инструкц|текст|код|подборк|рацион|диет|маршрут|сценар)|как\s+(приготовить|сделать|пожарить|сварить|выучить|накачать|похудеть)/i;

const DEFERRAL_RE = // ответ-отговорка: обещание/перенос без содержания
  /(сейчас|щас|момент(ик)?|секунд|подожди|попозже|позже|чуть\s*поз|дай\s*мне\s*(пар[уы]|секунд|минут)|состав(лю|им)|накида[юе]м?|скину|подготовлю|напишу тебе|уже пишу|готовлю|потом|завтра|с\s*утра|ночь\s*глядя|поздно уже|не\s*время)/i;

// Есть ли в ответе реальное содержание (список/много строк/развёрнутый текст).
function hasSubstance(s) {
  const t = String(s || '').trim();
  if (t.length > 300) return true;
  if (/(^|\n)\s*(\d+[.)]|[-–•*])\s+\S/.test(t)) return true; // маркеры списка
  return (t.match(/\n/g) || []).length >= 2; // несколько строк
}

// Пустая отговорка на запрос контента: обещал/отложил/переспросил, но ничего не дал.
export function isContentDeferral(text, reply) {
  if (!CONTENT_REQUEST_RE.test(text)) return false;
  const r = String(reply || '').trim();
  if (!r) return true;
  if (hasSubstance(r)) return false;
  if (DEFERRAL_RE.test(r)) return true;
  // короткий встречный вопрос вместо содержания тоже считаем отговоркой
  if (r.length < 170 && /\?\s*$/.test(r)) return true;
  return false;
}
    ...
    const forced = await ask(
      [
        { role: 'system', content: sys },
        {
          ...
          content:
            baseCtx +
            `\n\nЗапрос ${who}: «${text}»\n\nВыдай ПОЛНЫЙ готовый результат прямо в этом сообщении, целиком и структурно (списком, где уместно). НЕ откладывай, НЕ переспрашивай, НЕ отговаривайся временем суток - прими разумные значения по умолчанию и дай содержание сразу.${langHint}`,
        },
      ],
      { maxTokens: 1800, timeoutMs: 30000, retryDelays: [0, 5000] }
    );
...
// В личке - дневник дня, в группе - выжимка по чату (кто о чём, что решили, что висит).
const DIARY_TASK =
...
const GROUP_TASK =
  '\n\nПодведи итоги по чату за сегодня. Структура: «💬 О чём базарили» - 2-4 пункта по темам, с именами кто что сказал; «✅ Что решили» - договорённости (пропусти, если не было); «⏳ Что висит» - открытые дела и долги (пропусти, если пусто). Имена бери ТОЛЬКО из списка участников выше: имя, @ник и прозвище одного человека - это один человек, не разводи его на двоих. Кого нет в списке - того нет в чате. Людей называй ИМЕНАМИ из списка, а НЕ @никами: «@meloch287» в записях - это тот же человек, что и его имя, пиши имя. Раздел, в котором нечего сказать, ПРОПУСКАЙ целиком - не пиши «пусто» и не оставляй пустой заголовок. Заголовки пиши ровно как выше, с заглавной буквы и эмодзи - манера общения касается только строк под ними. Если за сегодня тишина - так и скажи, коротко.';
...
export async function aiDiarySummary(store, chatId, now = new Date(), onDelta = null) {
  ...
  return ask(
    [
      { role: 'system', content: friendSystem(user) },
      { role: 'user', content: context + (user?.isGroup ? GROUP_TASK : DIARY_TASK) },
    ],
...
// Реэкспорт функций из ai-skills.mjs именованными const (не `export * from`),
// чтобы refaudit.test.mjs видел каждое имя в этом файле. Просто ссылки.
import {
  aiChartSpec as _aiChartSpec,
  aiBirthday as _aiBirthday,
  aiSplit as _aiSplit,
  aiFitnessProgram as _aiFitnessProgram,
  aiRelay as _aiRelay,
  aiExtractReceipt as _aiExtractReceipt,
  aiConsolidate as _aiConsolidate,
  aiTts as _aiTts,
...
export const aiChartSpec = _aiChartSpec;
export const aiBirthday = _aiBirthday;
export const aiSplit = _aiSplit;
export const aiFitnessProgram = _aiFitnessProgram;
export const aiRelay = _aiRelay;
export const aiExtractReceipt = _aiExtractReceipt;
export const aiConsolidate = _aiConsolidate;
