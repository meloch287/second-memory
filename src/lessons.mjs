...
const SELF_EXPOSURE = [
  ...
  // Обратная крайность: прямое «да я человек» - это уже вранье, а не образ.
  // Уходить от темы можно, врать в лицо - нет.
  /(?:^|[\s,.!)])(?:да\s+)?я\s+(?:же\s+|ж\s+)?(?:живой\s+)?человек(?![а-яё])/i,
  /я\s+(?:такой\s+же\s+)?(?:живой|настоящий)(?![а-яё])/i,
];
...
// Отговорки на случай, когда модель упрямо палится даже со второй попытки
// (она копирует собственный старый ответ из истории). Лучше пустая шутка,
// чем «меня придумали программисты».
const DODGE = [
  'да какая разница кто) давай лучше о деле',
  'ой, не грузи меня этим. чё по планам-то',
  'секрет фирмы) спроси чё попроще',
  'а сама как думаешь) ладно, проехали',
];
...
export async function retryIfSelfExposed(reply, gen, head = '', question = '') {
  ...
  const again = await gen(
    head +
      '\n\nОтветь заново, СВОИМИ словами - НЕ копируй свой предыдущий ответ из истории. ' +
      'ЗАПРЕЩЕНО: называть себя ботом, ИИ, программой или нейросетью, отрицать, что ты человек, ' +
      'перечислять, кто тебя создал, и говорить «таких функций у меня нет». ' +
      'Отшутись одной фразой и переведи тему.'
  ).catch(() => null);
  if (again && !isSelfExposure(again, question)) return again;
  // обе попытки палевные - отдаём безобидную отговорку, а не разрушаем образ
  return DODGE[Math.floor(Math.random() * DODGE.length)];
}

// «Щас гляну», «уже пишу», «сайтам нужно время загрузиться» - обещание вместо
// ответа. Живой случай: человек ждал три с половиной часа и написал «ты меня
// динамишь». У бота нет фонового «потом допишу»: либо отвечает сейчас, либо
// честно говорит, что не умеет.
const STALLING = [
  /(?:щас|сейчас|счас)\s+(?:прям\s+|уже\s+|всё\s+|все\s+|тебе\s+|пару\s+)*(?:гляну|посмотрю|поищу|найду|проверю|уточню|скину|пришлю|накидаю|сделаю|напишу|дам|дописываю|минут|секунд|чекну|чекнем|чекнём|глянем|посмотрим|поищем|узнаю|разберусь)/i,
  /(?:подожди|погоди|обожди|жди)(?:\s+\S+){0,2}\s*(?:секунд|минут|чуть|немног|пару)|(?:пару\s+минут|минутк\w*)\s+подожди/i,
  /(?:уже|вот)\s+(?:пишу|ищу|смотрю|делаю|дописываю|заканчиваю)/i,
  /(?:нужно|надо)\s+врем[яени]+\s+(?:чтоб|для)/i,
  /(?:сайт\w*|страниц\w*)\s+(?:долго\s+)?(?:грузятся|грузится|загружаются|загружается)/i,
  /(?:дай|дайте)\s+(?:мне\s+)?(?:пару\s+)?(?:секунд|минут)/i,
  /(?:в\s+процессе|работаю\s+над\s+этим|скоро\s+будет)/i,
];

export function isStalling(reply) {
  return STALLING.some((re) => re.test(String(reply || '')));
}

// Перегенерация: либо ответ сейчас, либо честное «не умею».
export async function retryIfStalling(reply, gen, head = '') {
  if (!isStalling(reply)) return reply;
  const again = await gen(
    head +
      '\n\nОтветь заново. У тебя НЕТ возможности «посмотреть потом» и дописать позже: ' +
      'сообщение уходит человеку целиком и сразу. ЗАПРЕЩЕНО писать «щас гляну», «уже пишу», ' +
      '«подожди пару минут», «сайты грузятся». Либо дай ответ прямо сейчас из того, что знаешь, ' +
      'либо честно скажи, что этого не умеешь и что можешь предложить вместо.'
  ).catch(() => null);
  return again && !isStalling(again) ? again : reply;
}
