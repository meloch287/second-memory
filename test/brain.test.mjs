...
// U1: команду «Итог»/«саммари» из бота убрали - слово «саммари», сказанное
// в чат, больше не триггерит отдельный ИИ-саммари, а ведёт себя как обычная
// заметка (aiSummary/aiDiarySummary остались в ai.mjs для веба, но отвязаны
// от разбора текста).
test('U1: «саммари» в чате — обычная заметка, не команда саммари', async () => {
  delete process.env.AI_API_KEY;
  const s = freshStore();
  const r = await handleMessage(s, 'саммари', NOW);
  assert.doesNotMatch(r.reply, /не настроено/);
  assert.match(r.reply, /Сохранил заметку/);
});
...
test('«очисти память» - это команда очистки, а не заметка', async () => {
  const { parseMessage } = await import('../src/parser.mjs');
  assert.equal(parseMessage('очисти память').kind, 'wipe'); // полное стирание памяти
  assert.equal(parseMessage('очистить память').kind, 'wipe');
  assert.equal(parseMessage('сотри переписку').kind, 'clearchat'); // переписка - только история
  assert.equal(parseMessage('очисти чат').kind, 'clearchat');
  // «сотри из памяти X» / «удали из памяти X» остаются forget (точечное забывание)
  assert.equal(parseMessage('удали из памяти Петрова').kind, 'forget');
});

test('memoryStats и записи скоупятся по chatId (веб = свой юзер, не глобально)', async () => {
  const { memoryStats } = await import('../src/ragmeter.mjs');
  const s = freshStore();
  s.addFacts([{ chatId: 'owner', text: 'факт владельца' }, { chatId: 'owner', text: 'второй' }]);
  s.addFacts([{ chatId: 'g1', text: 'факт группы' }]);
  s.add({ chatId: 'owner', type: 'note', title: 'моя заметка' });
  s.add({ chatId: 'g1', type: 'note', title: 'групповая' });
  assert.equal(memoryStats(s, 'owner').counts.facts, 2, 'считаем только факты owner');
  assert.equal(memoryStats(s, 'owner').counts.entries, 1, 'только записи owner');
  assert.equal(memoryStats(s, 'g1').counts.facts, 1);
  assert.equal(memoryStats(s, null).counts.facts, 3, 'null - глобально');
  assert.equal(s.list({ chatId: 'owner' }).length, 1, 'list фильтрует по chatId');
});

test('«очисти память» реально стирает память чата и держит 0 (не восстанавливается)', async () => {
  const { memoryStats } = await import('../src/ragmeter.mjs');
  const { parseMessage } = await import('../src/parser.mjs');
  assert.equal(parseMessage('очисти память').kind, 'wipe');
  assert.equal(parseMessage('сотри всю память').kind, 'wipe');
  assert.equal(parseMessage('очисти чат').kind, 'clearchat', 'чат - только история');
  const s = freshStore();
  s.addFacts([{ chatId: 'web', text: 'ф1' }, { chatId: 'web', text: 'ф2' }, { chatId: 'tg', text: 'чужой' }]);
  s.add({ chatId: 'web', type: 'debt', counterparty: 'Дима', amount: 5000, direction: 'in' });
  s.addRaw('web', 'сырьё');
  const r = await handleMessage(s, 'очисти память', NOW, 'web');
  assert.equal(r.cleared, true);
  const c = memoryStats(s, 'web').counts;
  assert.equal(c.facts, 0); assert.equal(c.entries, 0);
  assert.equal(s.data.raw.filter((x) => x.chatId === 'web').length, 0, 'сырьё тоже стёрто - worker не пересоберёт');
  assert.equal(s.data.facts.filter((f) => f.chatId === 'tg').length, 1, 'чужой чат не тронут');
});

test('stripFakeSave: срезает фейковое «Сохранил заметку», живую реплику не трогает', async () => {
  const { stripFakeSave } = await import('../src/ai.mjs');
  assert.equal(stripFakeSave('Привет! Я Серега. Сохранил заметку: «Ку».'), 'Привет! Я Серега.');
  assert.equal(stripFakeSave('Ку! Как дела?'), 'Ку! Как дела?');
