...
test('пароль: установка, проверка, смена', async () => {
  const s = freshStore();
  ensureAuth(s, 'first-pass');
  assert.ok(await verifyPassword(s, 'first-pass'), 'верный пароль проходит');
  assert.ok(!await verifyPassword(s, 'wrong'), 'неверный не проходит');
  assert.ok(!await verifyPassword(s, ''), 'пустой не проходит');
  setPassword(s, 'second-pass');
  assert.ok(!await verifyPassword(s, 'first-pass'), 'старый больше не проходит');
  assert.ok(await verifyPassword(s, 'second-pass'), 'новый проходит');
  // ensureAuth не перетирает уже заданный пароль
  ensureAuth(s, 'third-pass');
  assert.ok(await verifyPassword(s, 'second-pass'), 'ensureAuth не сбрасывает существующий');
});

test('пароль: без WEB_PASSWORD генерируется случайный (не статический change-me)', async () => {
  const s = freshStore();
  ...
  assert.ok(await verifyPassword(s, r.password), 'сгенерированный пароль подходит');
  assert.ok(!await verifyPassword(s, 'change-me'), 'старый статический дефолт НЕ работает');
  ...
  assert.ok(await verifyPassword(s2, 'from-env'));
});

test('mustChangePass: виден в настройках и гаснет после смены пароля', () => {
  const s = freshStore();
  ensureAuth(s); // сгенерирован временный пароль
  assert.equal(getWebSettings(s).mustChangePass, true, 'флаг отдаётся в веб-настройки');

  setPassword(s, 'my-new-strong-pass');
  assert.equal(getWebSettings(s).mustChangePass, false, 'смена пароля гасит флаг');
  assert.equal(s.data.meta.web.mustChangePass, undefined, 'флаг удалён из меты');

  // env-пароль — флага нет с самого начала
  const s2 = freshStore();
  ensureAuth(s2, 'from-env');
  assert.equal(getWebSettings(s2).mustChangePass, false);
});
