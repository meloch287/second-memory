...
function friendSystem(user) {
  ...
  if (user?.isGroup) {
    return (
      ...
      'НЕ обещай того, чего не делаешь: не пиши «уже создаю» или «уже занимаюсь», если действие ещё не выполнено. Действие происходит только по чёткой команде (например «создай топик X») - если её не было, честно попроси написать команду, а не изображай работу. ' +
  ...
  return (
    ...
    'Если тебя дразнят, подкалывают, обзывают в шутку или просят дурашливое - не теряйся и НЕ отвечай «я не понимаю»: подыграй с юмором, отшутись, можешь любя подколоть в ответ, как живой друг. ' +
