...
const HELP = [
  'Я — ваша вторая память. Пишите обычным языком, я сам разложу по полочкам.',
  '',
  'Записать:',
  '• «Иванов должен 50 000 до 20 июля» — долг',
  '• «я должен Петрову 15к до пятницы» — ваш долг',
  '• «запиши встречу с Сергеем завтра в 15:00» — встреча',
  '• «напомни оплатить хостинг через 3 дня» — задача',
  '• «клиент Альфа просил расширить договор» — заметка',
  '',
  'Спросить:',
  '• «отгрузи все долговые обязательства заказчиков»',
  '• «сколько мне должны»',
...
async function route(store, text, now) {
  ...
  switch (p.kind) {
    case 'empty':
      return {
        reply:
          'Напишите, что записать или показать. Например: «Иванов должен 50 000 до 20 июля» или «покажи все долги». Команда «помощь» — все примеры.',
      };
    case 'help':
      return { reply: HELP };
    case 'summary': {
...
function digest(store, range, now) {
  ...
  if (!range) {
    ...
    if (!open.length) return { reply: 'Пока пусто. Напишите что-нибудь — я запомню. Команда «помощь» — примеры.' };
