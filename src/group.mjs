// Групповой режим: Толик живёт в группе как участник и помнит, кто что сказал
...
export function createGroupHandler(deps) {
  const { api, send, esc, store, log, withTyping, handleIntent, sendSummary, askReset, readDoc, downloadBase64, sleepyText, maybeReact, deliver, audioChoice, isAudioFile, audioInfo } = deps;
  ...
  async function groupFlow(msg) {
    ...
    if (msg.new_chat_members?.some((u) => u.id === botId)) {
      return send(
        chatId,
        `Здарова, я Толик 🙂 Теперь тусуюсь тут с вами\n\nЗапоминаю, кто что сказал и о чём договорились: дела, долги, встречи. Спросить - тегни @${botUsername}, например «@${botUsername} что решили по бюджету?»\n\nКидайте мне голосовые и аудиофайлы - расшифрую или сделаю саммари. Файлы (PDF, DOCX) тоже читаю\n\nАдминам: «закрепи» ответом на сообщение, «кикни», «замуть на час», «переименуй в ...», «дай ссылку»\n\nСделайте меня админом, чтобы я видел все сообщения`
      );
    }
    ...
    // Аудиофайл в группе (mp3/m4a/wav): спрашиваем, что сделать - расшифровку
    // или саммари. Голосовые ниже идут прежним путём.
    if (audioChoice && isAudioFile?.(msg) && audioEnabled()) {
    ...
    if (rawText.startsWith('/')) {
      ...
      if (slash === '/help' || slash === '/start') {
        return send(
          chatId,
          `Я Толик, ваш кент в этом чате 🙂 Слежу за движухой, запоминаю дела, долги и договорённости\n\nТегни @${botUsername} и спрашивай: «что решили по бюджету?», «баланс», «траты», «найди про...», «нарисуй график трат», «тегни Никиту», «тегни всех», «устрой опрос: пицца или суши», «кто сколько скинул на подарок?»\n\n🎧 Войс или кружок - расшифрую сразу. Аудиофайл (мп3, m4a, wav) - спрошу, что сделать: расшифровку или саммари. PDF и DOCX читаю сам\n\n📋 /summary - итоги по чату\n\nАдминам: «закрепи» ответом, «кикни», «замуть на час», «переименуй в ...», «дай ссылку». /reset - стереть память группы\n\nХочешь, чтобы знал, что было ДО меня - выгрузи историю в Telegram Desktop (JSON) и кинь сюда result.json`
        );
      }
    ...
    if (call && call.who !== '*') {
      ...
      // Имя не всегда первое слово: «разбуди толик аню», «кликни на серегу».
      // Перебираем кандидатов по порядку и берём первого, кто есть в реестре.
      let hit = null;
      for (const cand of call.candidates || [who]) {
        hit = findMember(g.members, cand);
        if (hit) break;
      }
      if (!hit) {
